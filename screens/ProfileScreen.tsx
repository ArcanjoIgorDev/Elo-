...
const ProfileScreen: React.FC<ProfileScreenProps> = ({ targetUserId, onBack, onSettings, onStartChat }) => {
  ...
  // UI State
  const [activeTab, setActiveTab] = useState<'overview' | 'vibes' | 'map'>('overview');
  ...
  return (
    <div className={`h-full bg-zinc-950 flex flex-col relative overflow-hidden ${isBlocked ? 'grayscale' : ''}`}>
        ...
        <div className="absolute top-0 left-0 w-full z-50 p-4 pt- safe flex justify-between items-center">
            <button onClick={onBack} className="w-10 h-10 flex items-center justify-center bg-black/20 backdrop-blur-xl rounded-full text-white border border-white/10 hover:bg-white/10 transition-colors">
                <ArrowLeft size={20} />
            </button>
            {isOwnProfile && (
                 <div className="flex gap-2">
                     {isEditing ? (
                         ...
                         <>
                            <button onClick={() => setIsEditing(true)} className="w-10 h-10 flex items-center justify-center bg-black/20 backdrop-blur-xl rounded-full text-zinc-200 hover:text-white border border-white/10"><Edit2 size={16} /></button>
                            <button onClick={onSettings} className="w-10 h-10 flex items-center justify-center bg-black/20 backdrop-blur-xl rounded-full text-zinc-200 hover:text-white border border-white/10"><Settings size={16} /></button>
                         </>
                     )}
                </div>
            )}
        ...
        <div className="flex-1 overflow-y-auto z-10 no-scrollbar pt-20">
            <div className="px-6 flex flex-col items-center">
                ...
                {/* Name & Bio */}
                <h1 className="text-3xl font-bold text-white tracking-tight mb-1">{profileUser.name}</h1>
                ...
                {isEditing ? (
                    <div className="w-full space-y-3 mb-6 relative">
                                    ...
                                    <div className="absolute top-full left-0 w-full mt-1 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl overflow-hidden animate-fade-in z-30 max-h-48 overflow-y-auto">
                                        ...
                                        {locationSuggestions.map((loc, i) => (
                                            <button 
                                                ...
                                                className="w-full text-left px-4 py-3 text-xs text-zinc-300 hover:bg-zinc-800 hover:text-white border-b border-white/5 last:border-0 transition-colors flex items-center gap-2 group"
                                            >
                                                <MapPin size={12} className="text-zinc-500 group-hover:text-brand-primary transition-colors"/> 
                        ...
                        {isOwnProfile && (
                            <button onClick={handleForceGPS} className="w-full py-3 bg-zinc-900 border border-zinc-800 rounded-xl text-xs text-brand-secondary/80 font-bold flex items-center justify-center gap-2 hover:bg-zinc-800 mt-2">
                                <LocateFixed size={14} /> Usar GPS Atual (Substituir Texto)
                            </button>
                        )}
                    </div>
                ) : (
                    <>
                        ...
                        <div className="flex gap-4 mb-8 text-xs text-zinc-500">
                            {profileUser.city && (
                                <div className="flex items-center gap-1.5 bg-zinc-900/50 px-3 py-1.5 rounded-full border border-zinc-800/50 text-brand-primary/80">
                                    <MapPin size={12} className="text-brand-primary"/> {profileUser.city}
                                </div>
                            )}
                            {profileUser.education && (
                                <div className="flex items-center gap-1.5 bg-zinc-900/50 px-3 py-1.5 rounded-full border border-zinc-800/50">
                                    <GraduationCap size={12} className="text-brand-accent"/> {profileUser.education}
                                </div>
                            )}
                        </div>
                    </>
                ...
                {/* Interaction Buttons (Not Owner) */}
                {!isOwnProfile && !isBlocked && (
                    <div className="flex gap-3 mb-8 w-full justify-center">
                        {friendshipStatus === 'accepted' ? (
                            <button onClick={() => onStartChat && onStartChat(profileUser)} className="bg-white text-black px-8 py-3.5 rounded-2xl font-bold text-sm shadow-[0_0_20px_rgba(255,255,255,0.15)] hover:scale-105 transition-transform flex items-center gap-2">
                ...
                {/* Content Tabs */}
                <div className="w-full flex p-1 bg-zinc-900/50 border border-zinc-800 rounded-xl mb-6 backdrop-blur-sm">
                    <button onClick={() => setActiveTab('overview')} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'overview' ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}>
                        <Grid size={14} /> Visão Geral
                    </button>
                    <button onClick={() => setActiveTab('vibes')} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'vibes' ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}>
                        <Layers size={14} /> Vibes
                    </button>
                    {!isOwnProfile && distanceKm !== null && (
                         <button onClick={() => setActiveTab('map')} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'map' ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}>
                            <Globe size={14} /> Neural Map
                        </button>
                    )}
                </div>
                    ...
                    {activeTab === 'overview' ? (
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div className="col-span-2 bg-zinc-900/40 border border-zinc-800/50 p-5 rounded-3xl relative overflow-hidden">
                                    <div className="flex justify-between items-center mb-4">
                                        <div>
                                            <h4 className="text-zinc-200 text-xs font-bold flex items-center gap-2"><BarChart3 size={14} className="text-brand-primary"/> Ressonância (7D)</h4>
                                        </div>
                        ...
                        <div className="animate-fade-in">
                            ...
                            <div className="w-full aspect-square bg-zinc-950 rounded-full border-2 border-zinc-800 relative overflow-hidden flex items-center justify-center shadow-[0_0_50px_rgba(16,185,129,0.05)] mb-4">
                                ...
                                <div className="absolute mt-8 text-[9px] font-bold text-zinc-500">VOCÊ</div>

                                {/* Target User (Simulated Relative Position for Visuals) */}
